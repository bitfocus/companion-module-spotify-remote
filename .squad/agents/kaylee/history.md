...
- **Volume variable real-time update during fade** (2026-05-19): Volume variable is updated during FadeVolume steps via `instance.setVariableValues({ volume: nextVolume })` in the try block after `setVolume` succeeds. No interface change needed — `setVariableValues()` is on `InstanceBase`.
